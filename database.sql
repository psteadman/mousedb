...
CREATE TRIGGER FCInsert
before insert on FC
for each row begin
select raise(abort, 'Insert on table "FC" violates Foreign Key Constraint "FCInsert" - ensure mouse is in database')
where (SELECT MouseID FROM Mouse WHERE MouseID = NEW.MouseID) is null;
end;
CREATE TRIGGER FCUpdate
before update on FC
for each row begin
select raise(abort, 'Update on table "FC" violates Foreign Key Constraint "FCUpdate" - ensure mouse is in database')
where (SELECT MouseID FROM Mouse WHERE MouseID = NEW.MouseID) is null;
end;
